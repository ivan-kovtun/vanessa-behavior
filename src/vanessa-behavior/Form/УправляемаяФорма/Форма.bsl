...
Функция СформироватьОписаниеОшибки(Знач Стр,ИмяФайла,ИмяПроцедуры) Экспорт
	...
	Стр = Стр + "; ИмяФайла="+ИмяФайла + ", ИмяПроцедуры="+ИмяПроцедуры + ", версия VB=" + ПолучитьВерсиюОбработкиСервер();
	Возврат Стр;
КонецФункции
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено) Экспорт
	...
	Если Объект.СоздаватьИнструкциюВидео Тогда
		ОчиститьСодержимоеКаталога(ПреобразоватьПутьСТочкамиКНормальномуПути(Объект.КаталогOutputИнструкцияВидео));
...
Процедура ЗапуститьСеансTestClient(ДопПараметр = "")
	...
	СтрокаЗапуска = """" + СтрокаЗапуска1с + """ ENTERPRISE " + КаталогБазы + ПараметрыОтладчика;
	
	// Временно фиксируем свободный порт
	Если ЭтоЗапускВРежимеКоманднойСтроки Тогда
		СтрокаЗапуска = СтрокаЗапуска + " /TESTCLIENT -TPort 1638 /CTemp " + ДопПараметр;//добавил /CTemp, чтобы убрать передачу параметра в TestClient, с которым был запущен TestManager
	Иначе
		СтрокаЗапуска = СтрокаЗапуска + " /TESTCLIENT -TPort 1638" + ДопПараметр;	
	КонецЕсли;	 
...
Функция СоздатьТестовоеПриложение()
	...
	// Временно фиксируем свободный порт
	ПараметрыОбъекта.Добавить("1638");
...
Процедура КаталогПроектаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		ДиалогВыбораКаталога.Показать(Вычислить("Новый ОписаниеОповещения(""КаталогВыборЗавершение"", ЭтотОбъект, ПараметрыОбъекта)"));
	Иначе
